...
void server::MoveXY(int x, int y, int lowx, int lowy) {
    try {
        ...
        if (path.size() > 0) {
            if (path.size() < 150) {
                for (std::size_t i = 0; i < path.size(); i++) {
                    ...
                    int vector_y = (path[i].second > path[i - 1].second) ? -1 : (path[i].second < path[i - 1].second) ? 1 : 0;
                    vec2.m_x = vec2.m_x * vector_x;
                    vec2.m_y = vec2.m_y * vector_y;

                    if (vector_y == -1) {
                        moveDirection = UPDATE_PACKET_FALLING_DOWN;
                    }
                    else {
                        if (vector_x == 1) {
                            moveDirection = UPDATE_PACKET_PLAYER_MOVING_RIGHT;
                        }
                        else if (vector_x == -1) {
                            moveDirection = UPDATE_PACKET_PLAYER_MOVING_LEFT;
                        }
                    }
                    ...
                    packet.m_vec2_x = vec2.m_x;
                    packet.m_vec2_y = vec2.m_y;
                    packet.m_state1 = -1;
                    packet.m_state2 = -1;
                    packet.m_packet_flags = moveDirection;
            ...
            packet.m_vec_x = pos.m_x;
            packet.m_vec_y = pos.m_y;
            packet.m_state1 = -1;
            packet.m_state2 = -1;
            ...
            g_server->local_player.pos.m_x = (float)((x * 32) + lowx);
            g_server->local_player.pos.m_y = (float)((y * 32) + lowy);

            variantlist_t varlist{ "OnTextOverlay" };

            varlist[1] = "`#Traveling " + to_string(path.size()) + " `0 Blocks ";;
